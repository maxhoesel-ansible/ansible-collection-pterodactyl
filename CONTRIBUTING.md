...
There are currently no modules in this collection, but interactions with Pterodactyls API (such as user or server management) are possible.
There is no ETA for any such features - if you'd like to implement them, feel free chat up @maxhoesel and discuss details.

## Developing Roles

The roles in this collection manage the installation of Pterodactyl software applications, such as the panel or the wings daemon.
...
- Try to design roles so that they are easily used together where possible and where reasonable (panel => wings interaction)
...
- [`./tests/test-roles`](./tests/test-roles) will use `molecule` to run tests on all roles in the collection (this might take a long time!)
    - To limit the scope to a single role, add a filter parameter: `./tests/test-roles pterodactyl_panel`
...
When writing module tests, you can start out by either copying an existing target such as [`caddy_config`](./tests/integration/targets/caddy_config/), or start out from scratch.
-->
...
Why not just `molecule-docker`? Simple: This collection needs systemd inside its test containers to start pterodactyl applications among other things, and modern versions of [systemd do not work inside docker containers anymore](https://github.com/ansible-community/molecule/discussions/3108).
...
- Pass a (partial) scenario name as a filter to limit execution (for example, `/tests/test-roles pterodactyl_panel` to limit molecule scenarios to the ACME role only)
